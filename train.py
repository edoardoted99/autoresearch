...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        # Expand k,v for GQA if n_kv_head < n_head
...
EMBEDDING_LR = 1.2      # learning rate for token embeddings (Adam) - increased 2x
UNEMBEDDING_LR = 0.008  # learning rate for lm_head (Adam) - increased 2x
MATRIX_LR = 0.08        # learning rate for matrix parameters (Muon) - increased 2x
SCALAR_LR = 1.0         # learning rate for per-layer scalars (Adam) - increased 2x
...
DEVICE_BATCH_SIZE = 16   # per-device batch size (reduced for MPS memory limits)
...
# torch.compile not supported on MPS — skip
...
while True:
    ...
    mfu = 100 * num_flops_per_token * TOTAL_BATCH_SIZE / dt / DEVICE_FP16_PEAK_FLOPS
...
steady_state_mfu = 100 * num_flops_per_token * TOTAL_BATCH_SIZE * (step - 10) / total_training_time / DEVICE_FP16_PEAK_FLOPS if total_training_time > 0 else 0
...
torch.save({'config': asdict(config), 'model_state_dict': model.state_dict()}, checkpoint_path)
